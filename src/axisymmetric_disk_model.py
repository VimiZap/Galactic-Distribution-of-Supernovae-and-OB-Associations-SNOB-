...
def integral_axisymmetric():
    ...
    print(type(integral))
    # when done with integrat   ing over r and b, return the value as we are not integrating over l
    return longitudes, np.array(integral) #/ (np.radians(1)) #* np.radians(5)) # devide by delta-b and delta-l in radians, respectively, for the averaging the paper mentions
...
def plot_axisymmetric():
    ...
    print(type(integrated_spectrum))

    integrated_spectrum *= 1/kpc**2 # convert from cm^-2 to kpc^-2
